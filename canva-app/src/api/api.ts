...
/**
 * Represents the response from palette transfer.
 */
interface PaletteTransferResult {
  /** The processed image URL. */
  processedImageUrl: string;
  /** Quality metrics for the transfer. */
  metrics: {
...
const endpoints = {
  ...
  paletteTransfer: "/api/v1/palette-transfer",
};
...
/**
 * Transfers palette between two images using Flask backend.
 * @param {Object} options - The options for palette transfer.
 * @param {File} options.sourceImage - The source image file.
 * @param {File} options.targetImage - The target image file.
 * @param {'A_to_B' | 'B_to_A'} options.direction - Transfer direction.
 * @returns {Promise<PaletteTransferResult>} - A promise that resolves to the transfer result.
 */
export const transferPalette = async ({
  sourceImage,
  targetImage,
  direction,
}: {
...
}): Promise<PaletteTransferResult> => {
  const url = new URL(endpoints.paletteTransfer, BACKEND_HOST);
  ...
  const result: PaletteTransferResult = await sendRequest(url, {
    method: "POST",
    body: formData,
  });
